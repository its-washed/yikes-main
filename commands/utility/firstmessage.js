...
module.exports = {
    data: { name: 'firstmessage', description: 'Get the first message in the channel', usage: ',firstmessage' },
    aliases: ['firstmsg', 'fm'],
    cooldown: 5,
    async execute(message) {
        ...
        return message.reply({
            embeds: [createEmbed({
                color: 0x6c5ce7, title: 'First Message',
                fields: [
                    { name: 'Author', value: first.author.tag, inline: true },
                    { name: 'Content', value: first.content || '*No content*', inline: false },
                    { name: 'Link', value: `[Click](${first.url})`, inline: true },
