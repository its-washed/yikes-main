...
module.exports = {
    data: { name: 'youtube', description: 'Track YouTube notifications', usage: ',youtube <add|remove|list> [args]' },
    cooldown: 5,
    async execute(message, args, client, config) {
        ...
        if (sub === 'add') {
            ...
            if (!user || !ch) return message.reply({ embeds: [errorEmbed('Usage', ',youtube add <username> #channel')] });
            const feeds = config.youtubeFeeds || [];
            ...
            updateGuildConfig(message.guild.id, { youtubeFeeds: feeds });
            return message.reply({ embeds: [successEmbed('Added', `Tracking **${user}** → ${ch}`)] });
        }
        if (sub === 'remove') {
            const user = args[1]; if (!user) return message.reply({ embeds: [errorEmbed('Usage', ',youtube remove <username>')] });
            const feeds = config.youtubeFeeds || []; const idx = feeds.findIndex(f => f.username.toLowerCase() === user.toLowerCase());
            if (idx === -1) return message.reply({ embeds: [errorEmbed('Not Found', 'Not tracking that user.')] });
            feeds.splice(idx, 1); updateGuildConfig(message.guild.id, { youtubeFeeds: feeds });
            return message.reply({ embeds: [successEmbed('Removed', `Stopped tracking **${user}**.`)] });
        }
        if (sub === 'list') {
            const feeds = config.youtubeFeeds || [];
            if (!feeds.length) return message.reply({ embeds: [createEmbed({ color: 0xff0000, title: 'YouTube Feeds', description: 'None configured.\nUse `,youtube add <username> #channel`.' })] });
            const pages = []; for (let i = 0; i < feeds.length; i += 8) { pages.push({ color: 0xff0000, title: `YouTube Feeds (${feeds.length})`, description: feeds.slice(i, i + 8).map((f, j) => `**${i + j + 1}.** \`${f.username}\` → <#${f.channelId}>`).join('\n') }); }
