...
module.exports = {
    data: { name: 'tiktok', description: 'Track TikTok notifications', usage: ',tiktok <add|remove|list> [args]' },
    cooldown: 5,
    async execute(message, args, client, config) {
        ...
        if (sub === 'add') {
            ...
            if (!user || !ch) return message.reply({ embeds: [errorEmbed('Usage', ',tiktok add <username> #channel')] });
            const feeds = config.tiktokFeeds || [];
            ...
            updateGuildConfig(message.guild.id, { tiktokFeeds: feeds });
            return message.reply({ embeds: [successEmbed('Added', `Tracking **${user}** → ${ch}`)] });
        }
        if (sub === 'remove') {
            const user = args[1]; if (!user) return message.reply({ embeds: [errorEmbed('Usage', ',tiktok remove <username>')] });
            const feeds = config.tiktokFeeds || []; const idx = feeds.findIndex(f => f.username.toLowerCase() === user.toLowerCase());
            if (idx === -1) return message.reply({ embeds: [errorEmbed('Not Found', 'Not tracking that user.')] });
            feeds.splice(idx, 1); updateGuildConfig(message.guild.id, { tiktokFeeds: feeds });
            return message.reply({ embeds: [successEmbed('Removed', `Stopped tracking **${user}**.`)] });
        }
        if (sub === 'list') {
            const feeds = config.tiktokFeeds || [];
            if (!feeds.length) return message.reply({ embeds: [createEmbed({ color: 0x000000, title: 'TikTok Feeds', description: 'None configured.\nUse `,tiktok add <username> #channel`.' })] });
            const pages = []; for (let i = 0; i < feeds.length; i += 8) { pages.push({ color: 0x000000, title: `TikTok Feeds (${feeds.length})`, description: feeds.slice(i, i + 8).map((f, j) => `**${i + j + 1}.** \`${f.username}\` → <#${f.channelId}>`).join('\n') }); }
